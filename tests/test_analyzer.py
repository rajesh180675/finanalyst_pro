...
class TestProductTableParsing:
    ...
    def test_parse_finished_products_with_thead_headers(self):
        html = b"""
        <html><body><table border="1">
        <thead>
            <tr>
                <th>Year</th><th>Product Name</th><th>Product Code</th><th>Unit of Measurement</th>
                <th>% of STO</th><th>Capacity Utilised -%</th><th>Installed Capacity</th>
                <th>Production</th><th>Sales Quantity</th><th>Sales</th><th>Sales Realisation/Unit -Unit Curr</th>
            </tr>
        </thead>
        <tbody>
            <tr><td>2025</td><td>Cigarettes (Million)</td><td>24022000</td><td>No</td><td>73.66</td><td>85.1</td><td>100</td><td>85.1</td><td>80.0</td><td>1332.92</td><td>16.66</td></tr>
        </tbody>
        </table></body></html>
        """
        parsed = parse_product_file(html, "products_table.xls")
        assert not parsed["finished_products"].empty
        row = parsed["finished_products"].iloc[0]
        assert int(row["year"]) == 2025
        assert row["product_name"] == "Cigarettes (Million)"
    ...
    def test_parse_raw_materials_with_thead_headers(self):
        html = b"""
        <html><body><table border="1">
        <thead>
            <tr>
                <th>Year</th><th>Product Name</th><th>Product Code</th><th>Unit of Measurement</th>
                <th>Product Quantity</th><th>Product Value</th><th>Cost/Unit -Unit Curr.</th>
            </tr>
        </thead>
        <tbody>
            <tr><td>2024</td><td>Aluminium Foils (Bobbins)</td><td>00011028</td><td>KG</td><td>120.0</td><td>784.16</td><td>6.53</td></tr>
        </tbody>
        </table></body></html>
        """
        parsed = parse_product_file(html, "raw_materials_table.xls")
        assert not parsed["raw_materials"].empty
        row = parsed["raw_materials"].iloc[0]
        assert int(row["year"]) == 2024
        assert row["product_name"] == "Aluminium Foils (Bobbins)"
        assert row["product_value"] == pytest.approx(784.16)
...
class TestSegmentFinanceParsing:
    ...
    def test_parse_segment_finance_html_thead_years_in_column_headers(self):
        html = b"""
        <html><body><table border='1'>
            <thead>
                <tr>
                    <th>Finance &gt;&gt;Segment Finance (Consolidated)&gt;&gt;ITC Ltd(Curr. Rs in ) Year</th>
                    <th>Finance &gt;&gt;Segment Finance (Consolidated)&gt;&gt;ITC Ltd(Curr. Rs in ) 202503</th>
                    <th>Finance &gt;&gt;Segment Finance (Consolidated)&gt;&gt;ITC Ltd(Curr. Rs in ) 202403</th>
                    <th>Finance &gt;&gt;Segment Finance (Consolidated)&gt;&gt;ITC Ltd(Curr. Rs in ) 202303</th>
                </tr>
            </thead>
            <tbody>
                <tr><td>REVENUE</td><td></td><td></td><td></td></tr>
                <tr><td>Revenue from Operations</td><td>73464.55</td><td>66657.04</td><td>69480.89</td></tr>
                <tr><td>AGRI BUSINESS</td><td>12244.00</td><td>8523.79</td><td>12361.62</td></tr>
            </tbody>
        </table></body></html>
        """
        out = parse_segment_finance_file(html, "SegmentFinance_.xls")
        assert not out.empty
        assert "AGRI BUSINESS" in set(out["segment"])
        assert "202503" in set(out["year"])
